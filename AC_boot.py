...
def train_actor_critic(env, num_episodes, lr, gamma, hidden_size, wandb_project, use_bootstrapping, use_baseline, lr_step_size, lr_gamma):
    ...
    policy_net = PolicyConv(
        env.observation_space.shape[0], hidden_size, env.action_space.n)
    ...
    scheduler_policy = optim.lr_scheduler.StepLR(
        optimizer_policy, step_size=lr_step_size, gamma=lr_gamma)
    scheduler_value = optim.lr_scheduler.StepLR(
        optimizer_value, step_size=lr_step_size, gamma=lr_gamma)
    ...
    for i_episode in range(num_episodes):
        ...
        while not done:
            ...
            if use_bootstrapping:
                ...
                ###########
                # isn't this the same as line 102?
                # where we use the next_value to calculate td_error?
                ###########
                target_value = reward if done else reward + gamma * next_value

            # fix shape of target_value to match value

            value_loss = mse_loss(torch.FloatTensor(
                [target_value]), value_net(torch.FloatTensor(state)))
            value_loss.backward()
        ...
        wandb.log({'total_reward': total_reward, 'policy_loss': policy_loss.item(), 'value_loss': value_loss.item(
        ), 'td_error': td_error, 'lr': scheduler_policy.get_last_lr()[0]})
        # print('Episode {}\tTotal reward: {:.2f} learning rate: {:.2e}'.format(i_episode, total_reward, scheduler_policy.get_last_lr()[0]))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train an Actor-Critic RL agent for the Catch environment')
    ...
    parser.add_argument('--wandb_project', type=str,
                        default='catch-rl-boot', help='Wandb project name')
    parser.add_argument('--use_bootstrapping', action='store_true',
                        help='use bootstrapping instead of Monte Carlo returns')
    parser.add_argument('--use_baseline', action='store_true',
                        help='use the value function as a baseline')
    parser.add_argument('--lr_step_size', type=int, default=50,
                        help='number of episodes before decreasing learning rate')
